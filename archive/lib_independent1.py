...
class IndependentEnergyNet1(Transformer):
    ...
    def energy_sgd(self, z, y_mask, x_states, x_mask, n_iter):
        ...
        for idx in range(n_iter):
            ...
            energy = self.compute_energy(z, y_mask, x_states, x_mask)  # [bsz]
            dummy = torch.ones_like(energy)
            dummy.requires_grad = True
            grad = autograd.grad(energy, z, create_graph=False, grad_outputs=dummy)
    ...
    def compute_loss(self, x, x_mask, y, y_mask):
        ...
        if OPTS.decgrad:
            logits = self.get_logits(z_ini, y_mask, x_states, x_mask)
            logp_mat = - F.cross_entropy(logits.reshape(-1, self.nmt()._tgt_vocab_size), y.flatten(),
                                       reduction="none").reshape(y.shape)
            logp = ((logp_mat * y_mask).sum(1)).sum()
            z_diff = autograd.grad(logp, z_ini, create_graph=False)[0]
        ...
        score = grad[0] # [bsz, targets_length, lat_size
        loss_mat = (score - z_diff) ** 2
        loss_mat = loss_mat.sum(2)
        loss = (loss_mat * y_mask).sum(1) / y_mask.sum(1)
        loss = loss.mean(0)
        # Compute log p of refined z
        # with torch.no_grad():
        refined_z = z_ini + score
        logits = self.get_logits(refined_z, y_mask, x_states, x_mask)
        logp_mat = F.cross_entropy(logits.reshape(-1, self.nmt()._tgt_vocab_size), y.flatten(), reduction="none").reshape(y.shape)
        target_logp = ((logp_mat * y_mask).sum(1) / y_mask.sum(1)).mean()
        y_pred = logits.argmax(-1)
        acc = (torch.eq(y, y_pred) * y_mask).sum() / y_mask.sum()

        score_map = {"loss": target_logp, "diffloss": loss, "cosine_sim": - target_logp, "tgtlogp": - target_logp, "acc": acc}
        return score_map
